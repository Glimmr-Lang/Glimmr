...
cp glimmr.lua $NVIM_CONFIG_DIR/lua/
cp -r syntax/* $NVIM_CONFIG_DIR/syntax

PACKAGE_INIT_STMT="\n-- auto-generated by \`install.sh\`\nrequire('glimmr').setup()"

if ! [[ -f "$NVIM_CONFIG_DIR/init.lua" ]]; then
  echo "Error: Unable to find init.lua in $NVIM_CONFIG_DIR."
  echo "please paste the following lua code into your configuratgion files in order to activate the glimmr package:"
  echo "    $PACKAGE_INIT_STMT"
  exit 0
fi

if grep -q "require('glimmr')" "$NVIM_CONFIG_DIR/init.lua" || grep -q "require(\"glimmr\")" "$NVIM_CONFIG_DIR/init.lua"; then
  echo "Note: Plugin already activated"
else
  echo -e $PACKAGE_INIT_STMT >>$NVIM_CONFIG_DIR/init.lua
fi

echo "glimmr NeoVim plugin successfully installed!"
